 ...
 */
export async function storeBundle(env: Env, machineId: string, bundle: SyncBundle): Promise<void> {
 ...
 */
export async function getBundle(env: Env, machineId: string): Promise<SyncBundle | null> {
  const key = `${KEY_PREFIX}${machineId}`;
 ...
 */
export async function deleteBundle(env: Env, machineId: string): Promise<void> {
  const key = `${KEY_PREFIX}${machineId}`;
