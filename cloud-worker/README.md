# OmniRoute Cloud Worker — راهنمای نصب و بهره‌برداری
...
OmniRoute Cloud Worker یک Cloudflare Worker است که به‌عنوان رله‌ی ابری برای OmniRoute محلی عمل می‌کند. این Worker:

- تنظیمات و اعتبارنامه‌های OmniRoute محلی را دریافت و ذخیره می‌کند (`POST /sync/:machineId`)
...
```
Deployed omniroute-cloud to https://omniroute-cloud.<your-subdomain>.workers.dev
```
...
## ۴. تنظیم OmniRoute محلی

در فایل `.env` یا تنظیمات محیطی OmniRoute:

```bash
# آدرس Worker (بدون / در انتها)
CLOUD_URL=https://omniroute-cloud.your-subdomain.workers.dev
...
سپس در داشبورد OmniRoute:

1. به **Settings → Cloud Sync** بروید
2. **Enable Cloud** را کلیک کنید
3. آدرس ابری نمایش داده می‌شود: `https://omniroute-cloud...workers.dev/<machineId>`
4. کلاینت‌های distant می‌توانند از این آدرس با کلید API OmniRoute استفاده کنند
...
curl -s https://omniroute-cloud.your-subdomain.workers.dev/<machineId>/v1/verify \
...
```bash
curl -s https://omniroute-cloud.your-subdomain.workers.dev/<machineId>/v1/chat/completions \
...
```bash
curl -N https://omniroute-cloud.your-subdomain.workers.dev/<machineId>/v1/chat/completions \
  -H "Authorization: Bearer sk_omniroute_..." \
  -H "Content-Type: application/json" \
  -d '{
    "model": "gpt-4o-mini",
...
```bash
curl -s https://omniroute-cloud.your-subdomain.workers.dev/<machineId>/v1/models \
  -H "Authorization: Bearer sk_omniroute_..." | python3 -m json.tool
```
...
۳. در OmniRoute محلی، `OMNIROUTE_CLOUD_SYNC_SECRET` را با همان مقدار به‌روزرسانی کنید
۴. OmniRoute را ریستارت کنید
...
- `POST /sync/:machineId` با موفقیت اجرا شده (لاگ OmniRoute را بررسی کنید)
...
- از همان کلید API استفاده می‌کنید که در داشبورد OmniRoute نمایش داده می‌شود
...
- `OMNIROUTE_CLOUD_SYNC_SECRET` در OmniRoute محلی با `CLOUD_SYNC_SECRET` در Worker یکسان است
...
- combo با fallback در OmniRoute محلی تنظیم کنید
