...
const ENV_VAR_ALLOWLIST = new Set([
  ...
  "VERBOSE",
  "LOG_LEVEL",
  "PORT", // generic, not RouteChi-specific
  "DATA_DIR",
  "REQUIRE_API_KEY",
  ...
  // subprocess — never read via `process.env.X` in RouteChi's own source, so the
  ...
  "OPENAI_BASE_URL", // env var RouteChi passes to downstream CLIs (AGENT_PROTOCOLS_GUIDE.md)
...
const ENDPOINT_ALLOWLIST = new Set([
  ...
  // paths on the UPSTREAM service (Claude.ai web, Blackbox), not RouteChi routes.
...
// Env var helper wrappers used across RouteChi — envInt(NAME, 5), envBool(NAME),
...
export function buildCodebaseIndex(root = ROOT) {
  ...
  // documented RouteChi env var (including external-CLI / docker / electron vars
