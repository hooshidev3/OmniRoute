 ...
 * `stream`. Without this hint, RouteChi previously routed those requests with
...
export function resolveStreamFlag(
  ...
  // RouteChi accumulates the provider stream and converts it to JSON for the client
  ...
  // RouteChi fallback would force SSE upstream and fail JSON-only providers as
  ...
  // No explicit stream param — preserve RouteChi's streaming default unless
