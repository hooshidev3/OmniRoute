...
/** Renders the X-RouteChi-Compression response header value. */
export function formatCompressionMeta(plan: DerivedPlan): string {
  return `${plan.mode}; source=${plan.source ?? "off"}`;
}

/**
 * Builds the annotation suffix for X-RouteChi-Compression from compression stats.
 ...
 * ASCII-only: this string is appended to the X-RouteChi-Compression HTTP header,
