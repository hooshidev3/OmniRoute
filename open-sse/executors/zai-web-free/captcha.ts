...
const ARG_PERM_TABLE = [
  32, 50, 10, 51, 6, 44, 37, 16, 46, 11, 62, 19, 43, 25, 23, 30, 60, 33, 53, 34, 7, 26, 12, 48, 5,
  2, 20, 4, 61, 13, 47, 49, 18, 29, 27, 22, 1, 17, 39, 56, 41, 38, 55, 31, 15, 58, 52, 40, 8, 57,
  45, 35, 59, 36, 42, 54, 63, 3, 24, 28, 14, 9, 0, 21,
];
...
function generateArg(certifyId: string): string {
  ...
  for (let i = 0; i < input.length;) {
    if (input[i] === 0x25 && i + 2 < input.length) {
      // '%'
      decoded.push(
        (fromHex(String.fromCharCode(input[i + 1])) << 4) |
          fromHex(String.fromCharCode(input[i + 2]))
      );
      i += 3;
    } else {
...
async function verifyCaptcha(
  ...
  if (respJson.Success && respJson.Result?.VerifyResult) {
    ...
    console.error(
      `[Captcha] verifyCaptcha: Success=true but securityToken/certifyId empty. Response: ${resp}`
    );
  } else if (respJson.Success) {
    console.error(
      `[Captcha] verifyCaptcha: VerifyResult=false for deviceToken=${deviceToken.slice(0, 20)}...`
    );
  } else {
...
export async function getCaptchaVerifyParam(
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    ...
    try {
      ...
      console.error(
        `[Captcha] tryCompute returned empty payload for token ${deviceToken.slice(0, 20)}...`
      );
    } catch (err) {
      // Token failed ?�� log the error and try the next one
      console.error(
        `[Captcha] tryCompute failed: ${err instanceof Error ? err.message : String(err)}`
      );
    }
  }
