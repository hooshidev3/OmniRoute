...
describe("transformToRouteChi", () => {
  test("converts LiteLLM per-token pricing to RouteChi per-million format", () => {
    const raw = {
      "openai/gpt-4o": {
    ...
    const result = transformToRouteChi(raw);
...
describe("pricing merge precedence", () => {
  test("user overrides > synced > defaults conceptual order", () => {
    ...
    const synced = transformToRouteChi(raw);
