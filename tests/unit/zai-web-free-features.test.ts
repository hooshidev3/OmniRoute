...
describe("zai-web-free peak hour retry", () => {
  ...
  it(
    "cooldown-aware retry maxRetries default is 10",
    {
      skip: "fork-specific tuning not yet applied to upstream's src/lib/resilience/settings.ts — will be revisited in a later commit",
    },
    async () => {
      const fs = await import("node:fs");
