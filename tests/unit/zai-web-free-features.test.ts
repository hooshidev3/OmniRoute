...
describe("zai-web-free features + reasoning", () => {
  it("resolveFeatures: enable_thinking defaults to true", async () => {
    ...
    const content = fs.readFileSync(
      "open-sse/executors/zai-web-free.ts",
      "utf-8"
    );
  ...
  it("resolveFeatures: 'think' is never in features", async () => {
    ...
    assert.ok(
      content.includes("delete features.think"),
      "'think' should be deleted from features"
    );
  ...
  it("<details> parsing exists in streaming path", async () => {
    const fs = await import("node:fs");
    const content = fs.readFileSync(
      "open-sse/executors/zai-web-free.ts",
      "utf-8"
    ...
    assert.ok(
      content.includes('raw.indexOf("<details")'),
      "Should parse <details> tags in streaming"
    );
    assert.ok(
      content.includes("stripDetailsTags") || content.includes("replace(/<details"),
      "Should strip <details> tags"
    );
  });

  it("<details> parsing exists in non-streaming path", async () => {
    const fs = await import("node:fs");
    const content = fs.readFileSync(
      "open-sse/executors/zai-web-free.ts",
      "utf-8"
    );
    // Check that <details> parsing exists after the non-streaming catch block
    ...
    assert.ok(
      afterCatch.includes('fullContent.indexOf("<details")'),
      "Should parse <details> in non-streaming path"
    );
  });
});
...
describe("zai-web-free captcha tuning", () => {
  it("token-collector has updated values", async () => {
    const fs = await import("node:fs");
    const content = fs.readFileSync(
      "open-sse/executors/zai-web-free/token-collector.ts",
      "utf-8"
    );
    assert.ok(
      content.includes("const MAX_TOKENS = 1500"),
      "MAX_TOKENS should be 1500"
    );
    assert.ok(
      content.includes("const DEFAULT_TOKENS = 850"),
      "DEFAULT_TOKENS should be 850"
    );
    assert.ok(
      content.includes("const DEFAULT_BATCH = 5"),
      "DEFAULT_BATCH should be 5"
    );
    assert.ok(
      content.includes("const SEND_WAIT_MS = 15000"),
      "SEND_WAIT_MS should be 15000"
    );
  });
  ...
  it("auto-refresh-daemon uses 850 tokens", async () => {
    ...
    assert.ok(
      content.includes("tokens: 850"),
      "auto-refresh should use 850 tokens"
    );
  });
});
...
describe("zai-web-free peak hour retry", () => {
  it("peak hour errors return 503 (retryable)", async () => {
    ...
    assert.ok(
      content.includes("return 503"),
      "Peak hour errors should return 503"
    );
  });

  it("cooldown-aware retry maxRetries default is 3", async () => {
    const fs = await import("node:fs");
    const content = fs.readFileSync(
  ...
  it("MAX_REQUEST_RETRY is 10 (hard cap)", async () => {
    ...
    assert.ok(
      content.includes("const MAX_REQUEST_RETRY = 10"),
      "MAX_REQUEST_RETRY should be 10"
    );
  });
});
