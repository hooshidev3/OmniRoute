...
OmniRoute می‌تواند سرور محلی خود (`http://localhost:20128`) را به اینترنت
...
هر سه backend درون فرایند مدیریت می‌شوند — OmniRoute باینری یا SDK زیرین را
...
- **نصب خودکار.** در اولین استفاده، OmniRoute جدیدترین باینری `cloudflared`
...
login` + `cloudflared tunnel route dns ...`) توسط
  OmniRoute هماهنگ نمی‌شوند. URLها موقتی‌اند و در هر راه‌اندازی تغییر می‌کنند.
...
| متغیر                                                | هدف                                                                                         |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| `CLOUDFLARED_BIN`                                    | بازنویسی مسیر باینری. اگر تنظیم و معتبر باشد، OmniRoute به‌جای دانلود از آن استفاده می‌کند. |
...
باید در داشبورد ngrok پیکربندی شوند — خود OmniRoute فقط
...
۱. Tailscale را نصب کنید (یا بگذارید OmniRoute این کار را انجام دهد — ر.ک. نقطهٔ پایانی `install` زیر).
۲. وارد شوید (`tailscale login` یا ازطریق نقطهٔ پایانی `login` در OmniRoute).
...
کش گذرواژهٔ MITM در OmniRoute (`getCachedPassword` / `setCachedPassword`) پاس داده می‌شود. ویندوز از نصب سرویس پیش‌فرض در
...
هنگامی که OmniRoute را ازطریق یک تونل در معرض دید قرار می‌دهید، داشبورد و جریان‌های OAuth باید
...
و OmniRoute را پیش از شروع OAuth راه‌اندازی مجدد کنید. برای تونل‌های موقتی Cloudflare Quick
...
برای پایش برنامه‌نویسی‌شده، نقاط پایانی `GET` هر backend را poll کنید. اجرای همزمان بیش از یک backend مجاز است؛ OmniRoute هرکدام را به‌صورت
مستقل ردیابی می‌کند.
...
OmniRoute تلاش می‌کند در اولین استفاده به‌صورت خودکار نصب کند. اگر نصب مسدود شده باشد
