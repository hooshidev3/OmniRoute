...
> **این سند چیست.** یک ارزیابی انتقادی از سیستم گیت کیفیت OmniRoute،
...
- **OmniRoute:** Prettier + ESLint ازطریق lint-staged (pre-commit)، ۲-فاصله/دو‌کوتیشن/۱۰۰ستون.
- **عمومی:** یک قالب‌بند auto-fixable + یک linter، در pre-commit روی فایل‌های staged.

### ۲. تایپ‌ها

- **OmniRoute:** `typecheck:core` (مسدودکننده) + `typecheck:noimplicit:core` (advisory) + ratchet `type-coverage` 92.17% + بودجهٔ any هر‌فایل.
- **عمومی:** typecheck سخت‌گیرانه در CI + معیار type-coverage با ratchet + بودجهٔ `any`/escape-hatch هر‌فایل.

### ۳. تست‌ها (شدت)

- **OmniRoute:** ۲ runner غیرهمپوشان (Node native + vitest)، ۸ shard، پوشش سراسری ۶۰/۶۰/۶۰/۶۰ + ratchet ~۷۶٪ + **۸ کف هر‌ماژول برای ماژول‌های بحرانی** + تست‌های property شبانه + **تست جهش** شبانه.
...
- **OmniRoute:** `pr-test-policy` (کد محصول نیازمند تست)، `check-test-masking` (assertهای تضعیف‌شده را مسدود می‌کند)، `pr-evidence` (ادعای موفقیت نیازمند بلوک evidence)، `test-discovery` (هر تست توسط یک runner جمع‌آوری می‌شود).
...
- **OmniRoute:** هشدارهای ESLint (3769↓)، تکثیر jscpd (5.72%↓)، پیچیدگی cyclomatic+max-lines (1800↓)، پیچیدگی شناختی sonarjs (753↓)، dead-code/unused-exports knip (339↓)، اندازهٔ فایل هر‌فایل (ثابت‌شده، فقط کوچک‌شونده)، circular-deps (Tarjan سفارشی، مسدودکننده).
...
- **OmniRoute:** CodeQL (ratchet هشدارها = ۰)، gitleaks (`[extend] useDefault=true` — حیاتی!)، SonarQube، قواعد امنیتی سفارشی (public-creds، error-helper، route-guard-membership، route-validation).
...
- **OmniRoute:** osv-scanner + npm-audit + Trivy + Dependabot (SCA)، license-checker (allowlist SPDX)، lockfile-lint (HTTPS+sha512+registry)، `check-deps` ضد slopsquatting (allowlist + عمر ≥۷۲h).
...
- **OmniRoute:** SBOM (CycloneDX + syft)، SLSA provenance (`--provenance`)، OpenSSF Scorecard (هفتگی)، سخت‌سازی workflow (zizmor: artipacked→`persist-credentials:false`، cache-poisoning، token-permissions).
...
- **OmniRoute:** oasdiff (تغییر breaker OpenAPI)، schemathesis (contract fuzz شبانه)، openapi-coverage (درصد مسیرهای مستند‌شده، ratchet 38.3%)، openapi-security-tiers (spec در برابر route-guard).
...
- **OmniRoute:** docs-sync (نسخه‌های آینه)، docs-counts-sync (اعداد در مستندات در برابر کد)، env-doc-sync، doc-links، fabricated-docs، cli-i18n، i18n-ui-coverage (`--threshold=65` + ratchet 80.1%).
...
- **OmniRoute:** known-symbols (دیسپچ رشته‌ای ⇒ نماد زنده)، provider-consistency، fetch-targets (fetch کلاینت ⇒ مسیر واقعی)، docs-symbols، db-rules (Hard Rules #2/#5)، migration-numbering.
...
- **OmniRoute:** chaos (تزریق خطا)، heap-growth (نشت)، k6 (soak)، promptfoo+garak (red-team LLM بر اساس OWASP LLM Top 10)، ۳ قانون تاب‌آوری (circuit-breaker/cooldown/lockout).
