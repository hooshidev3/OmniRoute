...
OmniRoute از پیکربندی پراکسی در **چهار اسکوپ مستقل** پشتیبانی می‌کند که به‌ترتیب اولویت حل می‌شوند:
...
هنگامی که OmniRoute درخواستی به یک ارائه‌دهندهٔ بالادست می‌فرستد، `resolveProxyForConnectionFromRegistry()` را فراخوانی می‌کند که هر سطح را به‌ترتیب بررسی می‌کند:
...
پراکسی‌ها در سیستم **Backup/Restore** گنجانده شده‌اند. هنگامی که پیکربندی OmniRoute را خروجی می‌گیرید:
...
اگر در نسخهٔ قدیمی (پیش از رجیستری) پراکسی پیکربندی کرده‌اید، OmniRoute به‌طور خودکار آن‌ها را مهاجرت می‌دهد:
...
> 🆕 **مشارکت‌شده توسط [@oyi77](https://github.com/oyi77)** — PR [#1847](https://github.com/borhandarabi/omniroute/pull/1847) (Issue [#1788](https://github.com/borhandarabi/omniroute/issues/1788))

OmniRoute با پلتفرم انجمنی **[1proxy](https://1proxy-api.aitradepulse.com)** یکپارچه می‌شود تا دسترسی به **صدها پراکسی رایگان و معتبر** از سراسر جهان را فراهم کند. این برای کاربرانی که زیرساخت پراکسی اختصاصی ندارند عالی است.
...
1. **همگام‌سازی** — OmniRoute پراکسی‌های معتبر را از API 1proxy دریافت می‌کند
...
OmniRoute فقط ترافیک را ازطریق پراکسی مسیریابی نمی‌کند — بلکه ترافیک را مشروع جلوه می‌دهد:
...
برای ارائه‌دهنده‌هایی که از الگوی CLIProxyAPI استفاده می‌کنند، OmniRoute سه حالت پراکسی بالادست پشتیبانی می‌کند:

| حالت          | توصیف                                                         |
| ------------- | ------------------------------------------------------------- |
| `native`      | OmniRoute مستقیماً مسیریابی پراکسی را مدیریت می‌کند (پیش‌فرض) |
...
برای در معرض دید عمومی قرار دادن نمونهٔ OmniRoute (Cloudflare/ngrok/Tailscale) به‌جای مسیریابی خروجی ازطریق پراکسی، ر.ک. [TUNNELS_GUIDE.md](./TUNNELS_GUIDE.md). REST API تونل در `/api/tunnels/{cloudflared,ngrok,tailscale}/*` قرار دارد و مستقل از زنجیرهٔ پراکسی خروجی است که در بالا مستند شده.
...
این با پراکسی‌های ارزان‌قیمتی که اتصال‌های بیکار را قطع می‌کنند طبیعی است. OmniRoute این را ازطریق موارد زیر مدیریت می‌کند:
...
مطمئن شوید پراکسی **پیش از** شروع جریان OAuth پیکربندی شده است. OmniRoute تبادل توکن OAuth را ازطریق پراکسی پیکربندی‌شده مسیریابی می‌کند. ابتدا یک پراکسی سراسری یا سطح ارائه‌دهنده تنظیم کنید، سپس اتصال دهید.
...
مکانیزم **شکست‌سریع پراکسی** OmniRoute (`src/lib/proxyHealth.ts`) پراکسی‌های مرده را در کمتر از ۲ ثانیه ازطریق یک بررسی سریع اتصال TCP تشخیص می‌دهد و سپس **نتیجه را کش می‌کند** تا از سربار هر درخواست جلوگیری شود.
...
OmniRoute استفادهٔ هر پراکسی را ردیابی می‌کند تا به اپراتورها در تشخیص الگوهای مسیریابی، جهش‌های تأخیر و شکست‌های مکرر کمک کند.

### چه چیزی ردیابی می‌شود

برای هر درخواست ازطریق یک پراکسی پیکربندی‌شده، OmniRoute ثبت می‌کند:
...
هنگامی که چندین پراکسی به یک اسکوپ تخصیص می‌یابند، OmniRoute از یک **استراتژی چرخش** برای انتخاب پراکسی برای هر درخواست استفاده می‌کند. استراتژی در سطح اسکوپ پیکربندی می‌شود (سراسری، هر ارائه‌دهنده، هر حساب، هر combo).
