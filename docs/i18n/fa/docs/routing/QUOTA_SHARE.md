...
**مشکلی که حل می‌کند:** OmniRoute کلیدهای API زیادی را در برابر یک حساب پروایدر بالادست یکسان پراکسی می‌کند. بدون منطق اشتراک‌گذاری، یک جهش از کلید A می‌تواند سهمیهٔ پروایدر برای آن ساعت را تخلیه کند و کلیدهای B و C را تا زمان بازنشانی پنجره مسدود کند.
...
- تمامی پایداری در پایگاه‌دادهٔ SQLite موجود OmniRoute ذخیره می‌شود (`DATA_DIR/storage.sqlite`).
