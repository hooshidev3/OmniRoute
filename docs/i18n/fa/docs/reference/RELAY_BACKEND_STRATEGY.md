...
OmniRoute اکنون سه حالت relay را برای `/api/v1/relay/chat/completions` پشتیبانی می‌کند:
...
مرز محصول پایدار، API relay OmniRoute است، نه پیاده‌سازی داشبورد. داشبورد Next.js ممکن است سرویس‌های محلی را نصب، پیکربندی و نظارت کند، اما مسیریابی درخواست باید از طریق API relay وارد شود و پشت آن مرز تحویل داده شود.
...
- از Bifrost به‌عنوان sidecar Tier-1 با throughput بالا ترجیحی استفاده کنید زمانی که استقرار نیاز به واریانس مسیریابی کمتر، چرخش متمرکز ارائه‌دهنده یا scale-out در میان چند replica از OmniRoute دارد.
