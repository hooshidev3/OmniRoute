...
backend serverless خاتمه می‌دهند تا OmniRoute بتواند از یک منطقه پایدار egress کند در حالی که
API key ارائه‌دهنده را سمت سرور نگه می‌دارد. این سند دو حالت شکست را پوشش می‌دهد
که اپراتورها در production با آن مواجه می‌شوند و مسیرهای بازیابی که OmniRoute برای
هرکدام عرضه می‌کند.
...
3. روی **Repair** کلیک کنید. OmniRoute فراخوانی می‌کند
...
headerهای آگاهی relay بازگردانده‌شده توسط probe تست خودکار OmniRoute را بررسی کنید
