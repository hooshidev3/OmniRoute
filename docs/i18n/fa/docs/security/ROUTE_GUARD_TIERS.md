...
همه مسیرهای API مدیریت OmniRoute در یکی از سه لایه حفاظت طبقه‌بندی می‌شوند. طبقه‌بندی ایستا است، در `src/server/authz/routeGuard.ts` تعریف شده، و پیش از اجرای هر شاخه احراز هویت دیگری ارزیابی می‌شود.
...
اپراتور OmniRoute را پشت یک تونل nginx/Cloudflare/Tailscale قرار داده و یک JWT
نشسته، یا احراز هویت اشتباه پیکربندی شده — مهاجم «فراخوانی یک API» را به «اجرای یک
فرمان روی میزبان» (اجرای کد از راه دور) تبدیل می‌کند. OmniRoute این را با اعمال یک
...
اگر OmniRoute را پشت یک پراکسی معکوس یا تونل (nginx, Caddy, Cloudflare
...
- یک `403 LOCAL_ONLY` در لاگ‌های OmniRoute برای یکی از این مسیرها، گیت است که
