...
دو جدول سیگنال **جدا** و مجاور در همان فایل قرار دارند که بخشی از شناسایی کلیدواژه‌های ممنوع _نیستند_:
...
- پایانى‌شدن دائمی `banned` هنگام وجود بدنه با سیگنال مسدودی در **هر وضعیت HTTP** رخ می‌دهد
  ...
  **`deactivated`** (`isActive=false` وقتی اتصال کلید API یدکی ندارد) توسط مسیر درون‌خطی
  ...
  مسیر `markAccountUnavailable()` یک وضعیت پایانى _متفاوت_ —
...
با این حال، _رشته‌های_ داخلی به سوی ارائه‌دهندگان اشتراک/OAuth با ریسک واقعی مسدودی
