...
رمزگشایی شفاف TPROXY **۵مین حالت ضبط** OmniRoute برای پشته
...
تغییر پیکربندی** نیاز ندارد — اما فرآیند باید به CA پویا که OmniRoute
...
OmniRoute به کار خود ادامه می‌دهد.
...
- یک CA طولانی‌عمر از طریق `generateMitmCa()` تولید می‌کند (CN `"OmniRoute MITM CA"`،
...
میزبان را امضا کند یعنی هر چیزی که OmniRoute رهگیری کند قابل رمزگشایی است. این پشت
...
حالت قواعد را برمی‌گرداند؛ اگر OmniRoute در وسط جلسه کشته شد، از عمل
