...
OmniRoute باید این مقادیر را embed کند تا کاربرانی که `.env` را پیکربندی نمی‌کنند، همچنان یک جریان OAuth کارآمد به‌صورت پیش‌فرض دریافت کنند. بدون یک fallback embed‌شده، ارائه‌دهندگان Gemini / Antigravity / Windsurf برای هر کاربری که از مسیر «فقط clone و اجرا» پیروی می‌کند از کار می‌ایستند.
