...
OmniRoute به هر پراکسی اجازه می‌دهد یک **دستورالعمل خروجی خانواده آدرس** داشته باشد. به‌طور پیش‌فرض، سیستم‌عامل IPv4 یا IPv6 را انتخاب می‌کند (دوگانه، «Happy Eyeballs»). وقتی دستورالعمل را روی `ipv4` یا `ipv6` تنظیم می‌کنید، OmniRoute هر اتصال از طریق آن پراکسی را به خانواده انتخاب‌شده پین می‌کند و به جای بازگشت به خانواده دیگر، **fail-closed** می‌شود.
...
معرفی‌شده در PR [#3777](https://github.com/borhandarabi/omniroute/pull/3777). مشکلات انگیزه:
...
وقتی `family` برابر `auto` است، OmniRoute **هیچ** دستورالعملی اضافه نمی‌کند — URL پراکسی همان‌طور که هست استفاده می‌شود و خانواده اتصال به‌صورت ذاتی تعیین می‌شود.
...
وقتی دیسپچر ساخته می‌شود، نشانگر خوانده شده و به یک خانواده اتصال ملموس تبدیل می‌شود. اگر میزبان یک IP literal از خانواده **مخالف** باشد، OmniRoute استثنا پرتاب می‌کند (تناقض fail-closed است):
...
پین خانواده با پراکسی‌های SOCKS5 کار می‌کند، اما `fetch-socks` استاندارد گزینه‌های سوکت مورد نیاز برای پین‌کردن خانواده hop پراکسی را افشا نمی‌کند. OmniRoute کانکتور خودش را برای این کار ارسال می‌کند:
