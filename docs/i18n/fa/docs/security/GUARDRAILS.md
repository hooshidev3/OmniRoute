...
OmniRoute و ارائه‌دهندگان آپ‌استریم اعمال می‌کنند. هر گاردریل می‌تواند payloadهای درخواست
...
- **`garak` (مشورتی)** — garak `--probes promptinject,dan,leakreplay`
  را بر علیه یک نمونه محلی OmniRoute (`http://localhost:20128/v1`) اجرا می‌کند. گیت‌شده بر یک
