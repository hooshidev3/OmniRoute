...
RouteChi 与三类 CLI 工具集成，分布在三个专用的 dashboard 页面中：

| 页面             | 路由                     | 概念                                                                        | 数量         |
| ---------------- | ------------------------ | --------------------------------------------------------------------------- | ------------ |
| **CLI Code's**   | `/dashboard/cli-code`    | 指向 RouteChi 的编程工具（客户端 → CLI → RouteChi → 服务商）               | 19           |
| **CLI Agents**   | `/dashboard/cli-agents`  | 指向 RouteChi 的自主代理（相同流程，更广泛的范围）                           | 6            |
| **ACP Agents**   | `/dashboard/acp-agents`  | RouteChi 通过 stdio/ACP 作为后端启动的 CLI（反向流程）                       | 见注册表      |
...
ACP Agents（反向启动流程）：
    客户端请求 → RouteChi → 通过 stdio/ACP 启动 CLI → 响应
```
...
无需手动编写每个工具的配置。RouteChi 为每个支持的 CLI 提供对应的 `setup-*` 命令，该命令从运行中的 RouteChi（本地或远程）读取**实时**模型目录并将工具的配置写入你的机器：
...
每个命令接受 `--remote <url> --api-key <key>`（针对远程 RouteChi 配置本地工具）、`--dry-run`（预览不写入）和 `--port`。不支持模型自动发现的工具（Cline、Kilo、Roo、Goose、Qwen、Aider、Gemini）接受 `--model <id>`（以及用于非交互式运行的 `--yes`）。启动器 `routechi launch`（Claude Code）和 `routechi launch-codex`（Codex）在注入正确的环境变量后启动 CLI，不写入任何配置。
...
此页面（从 `/dashboard/agents` 重命名而来）显示 RouteChi 可以通过 stdio/ACP 协议**启动**为后端执行引擎的 CLI。目录在 `src/lib/acp/registry.ts` 中单独维护，**不同于** `CLI_TOOLS`。
...
或使用 RouteChi dashboard → **CLI Tools → Cline → 应用配置**。
...
或使用 RouteChi dashboard → **CLI Tools → KiloCode → 应用配置**。
...
当 VS Code Insiders 配置了自定义端点模型，且你希望 RouteChi 在不使用自定义请求头字段的情况下工作时使用。
...
- 将 `sk-your-omniroute-key` 替换为在 RouteChi 中创建的 API Key。
...
# CLI 使用自己的认证 — Kiro CLI 本身不需要 RouteChi 作为后端。
# 将 kiro-cli 与 RouteChi 配合用于其他工具。
kiro-cli status
```

对于 **Kiro IDE** 桌面应用，使用 RouteChi 通过 `/dashboard/cli-tools → Kiro` 暴露的 MITM 端点。
...
> Qwen OAuth 免费层已于 2026-04-15 停用。改为使用 RouteChi 搭配
...
```bash
routechi providers available                       # RouteChi 服务商目录
...
> `providers available` 读取 RouteChi 目录；`providers list/test/test-all/validate`
...
以下命令假定 RouteChi 服务器正在运行（另有说明除外）：
...
routechi provider add                 # 将 RouteChi 注册为某个工具的服务商
...
可直接粘贴的示例（使用 Token 化 RouteChi URL）：
...
| 错误                                          | 原因                     | 修复方法                                          |
| --------------------------------------------- | ------------------------ | ------------------------------------------------- |
| `Connection refused`                          | RouteChi 未运行          | `routechi serve`                                 |
