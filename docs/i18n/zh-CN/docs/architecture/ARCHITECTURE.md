...
RouteChi 是基于 Next.js 构建的本地 AI 路由网关和控制台。
...
RouteChi 可以安装、监管并路由到本地运行的 AI 工具进程，称为**嵌入式服务**。
...
- RouteChi 响应元数据构建器：`src/domain/omnirouteResponseMeta.ts`
...
> **注意：** 下表是 RouteChi v3.8.0 中 237 个已注册服务商的代表性样本。
> 完整且持续更新的列表请参阅
...
- RouteChi 返回的最终客户端响应；流式响应以相同压缩摘要形式存储
