...
Panduan ini mendokumentasikan standar infrastruktur jaringan terbaik untuk mengamankan **RouteChi** dan mengekspos aplikasi Anda ke internet secara aman, **tanpa membuka satu pun port (Zero Inbound)**.

## Apa yang Telah Dilakukan pada VM Anda?

Kami mengaktifkan RouteChi dalam mode **Split-Port** melalui PM2:
...
3. Klik **Add a Tunnel**, pilih **Cloudflared**, dan beri nama `RouteChi-VM`.
...
2. Pilih **Self-hosted**.
3. Di **Application name**, masukkan `Panel RouteChi`.
...
3. Klik **Create rule**.
4. **Name:** `Anti-Penyalahgunaan RouteChi API`
...
2. RouteChi hanya berkomunikasi melalui HTTPS keluar (`cloudflared`) dan tidak menerima koneksi TCP langsung dari internet.
