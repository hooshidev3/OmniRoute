...
Este guia documenta o padrão ouro de infraestrutura de rede para proteger o **RouteChi** e expor sua aplicação de forma segura para a internet, **sem abrir nenhuma porta (Zero Inbound)**.

## O que foi feito na sua VM?

Nós ativamos o RouteChi em modo **Split-Port** através do PM2:
...
3. Clique em **Add a Tunnel**, escolha **Cloudflared** e dê o nome \`RouteChi-VM\`.
...
3. Em **Application name**, coloque \`Painel RouteChi\`.
...
4. **Name:** \`Anti-Abuso RouteChi API\`
...
2. O RouteChi só conversa HTTPS saindo (\`cloudflared\`) e não recebendo TCP direto do mundo.
