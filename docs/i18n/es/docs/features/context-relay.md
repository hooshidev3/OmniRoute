...
- before the active account is exhausted, RouteChi generates a compact structured summary
- after authentication selects a different account for the same session, RouteChi injects
...
If the active provider is enabled in `handoffProviders`, RouteChi generates a structured
...
RouteChi prepends the stored handoff as a system message. Injection happens only after the
...
At injection time, RouteChi converts that payload into a `<context_handoff>` system
