...
Traffic Inspector is RouteChi's built-in HTTPS traffic debugger — a Charles Proxy / mitmweb / HTTP Toolkit-like tool that is **LLM-aware** and **agent-aware**. It lives at `/dashboard/tools/traffic-inspector` and receives live traffic from up to 5 simultaneous capture sources.
...
| Feature                                                             | mitmweb | Charles | Fiddler | **RouteChi Traffic Inspector** |
| ------------------------------------------------------------------- | :-----: | :-----: | :-----: | :-----------------------------: |
| Web-based                                                           |    ✓    |    ✗    |    ✗    |                ✓                |
| Open-source                                                         |    ✓    |    ✗    | partial |                ✓                |
...
| **Integrated with RouteChi** routing, fallback, cost               |    ✗    |    ✗    |    ✗    |                ✓                |
...
interface LlmMetadata {
  ...
  costEstimateUsd: number | null; // estimated cost based on RouteChi pricing
}
```
...
If RouteChi crashes while system-wide proxy mode is active:
...
| POST   | `/requests/{id}/replay`     | Re-execute the same request through RouteChi router                               |
