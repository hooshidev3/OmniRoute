...
RouteChi exposes three different agent-related surfaces. They look similar at first glance but solve different problems. Use this page to pick the right one.
...
| **ACP — CLI Agents Registry** | Detecting / registering / launching CLI coding agents installed on the user's machine (Cursor, Cline, Codex CLI, Claude Code, Aider, etc.) | HTTP REST                   | RouteChi-specific   |
| **Cloud Agents**              | Submitting long-running coding tasks to external cloud services (Codex Cloud, Devin, Jules, Cursor Cloud)                                  | HTTP REST + DB-backed tasks | RouteChi-specific   |
...
**RouteChi endpoint:** `POST /a2a` (JSON-RPC 2.0)
**Agent Card:** `GET /.well-known/agent.json`

### When to use

- Building a multi-agent system where RouteChi is one of the peers
- Exposing RouteChi's routing intelligence (smart-routing, quota-management, etc.) to agents in frameworks like Google ADK or generic agent meshes
- Wrapping RouteChi behind a standard discovery + invocation surface
...
**RouteChi endpoint:** `GET /api/acp/agents`
**Source:** `src/lib/acp/{index,manager,registry}.ts`

### What it is

ACP is RouteChi's **local CLI agent inventory**. It detects which coding CLIs are installed on the host (Cursor, Cline, Claude Code, Codex CLI, Continue, etc.), resolves their versions, and surfaces them to the dashboard so the user can wire each CLI to point at RouteChi.
...
- Dashboard "CLI Tools" page lists what's installed and helps you point each at RouteChi
- Custom agents let power users register internal/proprietary CLIs that RouteChi doesn't know about by default
- Detection result fuels the `cli-tools` fingerprint matrix

### When NOT to use ACP

- ACP doesn't _run_ tasks. It only detects + configures CLIs. To actually invoke a CLI, you launch it yourself with the env vars RouteChi provides (`OPENAI_BASE_URL`, `OPENAI_API_KEY`, etc.).

## 3. Cloud Agents

**RouteChi endpoints:** `/api/v1/agents/tasks/*` (lifecycle) + `/api/cloud/*` (plumbing)
**Source:** `src/lib/cloudAgent/`

### What it is

A uniform interface over third-party cloud coding agents. You submit a prompt + repo URL, RouteChi dispatches to the right cloud agent, polls status, returns results.
...
| ------------------ | --------------------------------------------------------------------------------- | ---------------------------------------- |
| Standard           | Open A2A v0.3                                                                     | RouteChi-specific                       |
| Where compute runs | Inside RouteChi (uses configured combos)                                         | External (Codex / Devin / Jules servers) |
...
### Discover RouteChi's A2A capabilities
