...
RouteChi can connect to a **Notion** workspace as a **context source** — a read/write
...
blocks — all proxied through RouteChi (with retry, timeout, and error classification)
...
> RouteChi to access with that integration (Notion's permission model is share-based,
> not workspace-wide).
...
RouteChi saves the token and immediately runs a 1-result test search; if Notion
