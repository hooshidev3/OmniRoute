...
> **Audience:** Operators wiring OpenCode to an RouteChi deployment.
...
[OpenCode](https://opencode.ai) is an agentic CLI/desktop AI client. It reads its provider catalog from `~/.config/opencode/opencode.json` (or `opencode.jsonc`) and follows the schema at `https://opencode.ai/config.json`. RouteChi exposes itself to OpenCode as one of those providers — every request flows through RouteChi's standard OpenAI-compatible `/v1` surface, so OpenCode automatically benefits from Auto-Combo routing, circuit breakers, key policies, observability, etc.
...
Recommended for end users. Ships with RouteChi. Writes `opencode.json` in place.

```bash
# After installing RouteChi (npm i -g @omniroute/cli or local clone)
routechi config opencode \
  --baseUrl http://localhost:20128 \
  --apiKey "$OMNIROUTE_API_KEY"
```

Behind the scenes the CLI calls `mergeOpenCodeConfigText()` (`src/shared/services/opencodeConfig.ts:104`), so an existing `opencode.json` keeps its other providers and comments. The RouteChi entry is added/replaced atomically.
...
OpenCode UI/agent
   → @ai-sdk/openai-compatible
      → HTTP POST {baseURL}/chat/completions          (RouteChi OpenAI surface)
         → RouteChi /v1/chat/completions handler     (open-sse/handlers/chatCore.ts)
...
- `"auto"` — surfaces RouteChi's [Auto-Combo](../routing/AUTO-COMBO.md) zero-config router. Lets OpenCode pick "the best available model" without you hard-coding the catalog.
- `"<combo-name>"` — any combo you've defined in the dashboard; RouteChi resolves it transparently.
...
This deduplication is **the most common breakage** seen in older configs. If you have an `opencode.json` from before v3.8.0 that points at `/v1/v1/...`, re-run the generator or call `createRouteChiProvider` again.

---

## Authentication modes

| RouteChi setting                           | Recommended `apiKey` value                         |
| ------------------------------------------- | -------------------------------------------------- |
...
For Anthropic-style clients that send `x-api-key` + `anthropic-version`, RouteChi's `extractApiKey` also honours the key from `x-api-key`. OpenCode uses the OpenAI surface, so it'll always send `Authorization: Bearer ${apiKey}` — no Anthropic special-case applies here.
...
| `401 Invalid API key`                                | RouteChi has `REQUIRE_API_KEY=true` and the key is unknown.        | Create the key in the dashboard, or set `REQUIRE_API_KEY=false` (local only) and use `sk_omniroute`. |
| Model list empty in OpenCode UI                      | All 4 default models are hidden in RouteChi's provider visibility. | Pass `models: ["auto", ...]` to surface ones you've enabled.                                         |
...
- [API reference](../reference/API_REFERENCE.md) — full RouteChi REST surface
