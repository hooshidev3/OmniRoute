...
Delegated **Context Editing** is a Claude-only context-management feature. Unlike RouteChi's local
...
tool-use / tool-result blocks from its own running context window. RouteChi only attaches a body
parameter (`context_management.edits[]`); Claude does the actual clearing against its own tokenizer.

This is a delegated capability by nature: other providers reject the parameter, so RouteChi scopes
...
RouteChi injects a single edit into the outbound Anthropic Messages body:
...
RouteChi already emits on Claude requests.
...
left as-is. If a `clear_thinking_20251015` edit is also present, RouteChi stable-sorts the
...
After a Claude response, RouteChi records how much context the provider actually cleared. This is
...
| ----------------- | --------------------------------------------------- | ------------------------------------------- |
| Where it runs     | In RouteChi, before the request leaves the proxy   | In the provider (Claude), server-side       |
...
The two are complementary: local engines compress the bytes RouteChi sends; Context Editing lets
