# Auto-Combo: Let RouteChi Pick the Best AI for You

> **TL;DR**: Set your model to `auto` and RouteChi automatically picks the best AI provider for each request. No configuration needed.

---

## What It Does

Instead of choosing a specific AI model (like GPT-4o or Claude), you can let RouteChi **automatically pick the best one** for each request. It considers:
...
RouteChi scores all your connected providers and picks the best one. If it fails, it automatically tries the next one.
...
**Step 2**: That's it! RouteChi handles the rest.
...
When you send a request with `model: "auto"`, RouteChi:
...
4. **Auto-recovers** — If it fails, RouteChi tries the next one automatically
...
RouteChi has **three layers of protection**:

### 1. Auto-Fallback
If the best provider fails, RouteChi automatically tries the next one. You don't need to do anything.
...
If all providers fail, RouteChi routes to stable free providers (like Kiro or Qoder) as a last resort.

---

## Multi-Account Support

If you have multiple accounts for the same provider (e.g., two OpenAI keys), RouteChi treats each as a **separate candidate**. This means:
...
RouteChi occasionally **explores** new providers to discover better options:
...
This helps RouteChi learn which providers work best for your usage patterns.
...
RouteChi automatically skips it and tries the next one. If a provider keeps failing, it's excluded temporarily (5-30 minutes). You don't need to do anything.

### "Can I see which provider was used?"

Check the response headers — RouteChi includes the provider and model used in each response.
