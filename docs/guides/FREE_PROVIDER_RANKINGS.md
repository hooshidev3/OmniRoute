...
> **TL;DR**: RouteChi ranks its **free** providers by model quality using **Arena AI
...
RouteChi aggregates 160+ providers, many of which expose a **free tier** (no-auth,
...
Leaderboard categories map to RouteChi task categories:
...
- It is **non-blocking and never fatal** — if the upstream fetch fails, RouteChi keeps
...
stripped and a small alias map expands canonical names into the variants RouteChi uses
