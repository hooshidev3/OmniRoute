---
title: "RouteChi — Deployment Guide on VM with Cloudflare"
version: 3.8.40
lastUpdated: 2026-06-28
---

# RouteChi — Deployment Guide on VM with Cloudflare
...
Complete guide to install and configure RouteChi on a VM (VPS) with domain managed via Cloudflare.
...
Keep reverse-proxy stream timeouts aligned with your RouteChi timeout env vars. If you raise
`FETCH_TIMEOUT_MS` / `STREAM_IDLE_TIMEOUT_MS`, raise `proxy_read_timeout` / `proxy_send_timeout`
above the same threshold.

RouteChi uses `NEXT_PUBLIC_BASE_URL` as the canonical browser-facing origin for OAuth
...
`OMNIROUTE_TRUST_PROXY` if RouteChi is not directly reachable by clients and your proxy
...
| 20128 | RouteChi   | Localhost only (via nginx) |
