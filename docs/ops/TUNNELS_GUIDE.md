...
RouteChi can expose its local server (`http://localhost:20128`) to the public
...
All three backends are managed in-process — RouteChi starts/stops the underlying
...
- **Auto-install.** On first use, RouteChi downloads the latest `cloudflared`
...
login` + `cloudflared tunnel route dns ...`) are not orchestrated by
  RouteChi. URLs are ephemeral and will change every restart.
...
| Variable                                             | Purpose                                                                               |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------- |
| `CLOUDFLARED_BIN`                                    | Override the binary path. If set and valid, RouteChi uses it instead of downloading. |
...
must be configured in the ngrok dashboard — RouteChi itself only forwards the
...
1. Install Tailscale (or let RouteChi do it — see `install` endpoint below).
2. Sign in (`tailscale login` or via RouteChi's `login` endpoint).
...
RouteChi's MITM password cache (`getCachedPassword` / `setCachedPassword`) for
...
When you expose RouteChi through a tunnel, the dashboard and OAuth flows must
...
and restart RouteChi before initiating OAuth. For ephemeral Cloudflare Quick
...
than one backend simultaneously is allowed; RouteChi will track each
independently.
...
RouteChi attempts to auto-install on first use. If the install is blocked
