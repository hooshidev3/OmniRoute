 ...
 * can be iframed inside the RouteChi dashboard without CORS issues.
 *
 * Security:
 ...
 *     to prevent credential leakage between RouteChi and the embedded service.
 ...
 * Prevents RouteChi session cookies and Authorization from leaking upstream.
 */
 ...
 *   RouteChi origin, which would conflict with session management.
 * - content-security-policy / content-security-policy-report-only: the
 *   embedded service's CSP is irrelevant inside the RouteChi iframe.
 * - x-frame-options: would block the iframe entirely if set to DENY/SAMEORIGIN
 *   by the embedded service (RouteChi controls framing via its own CSP).
