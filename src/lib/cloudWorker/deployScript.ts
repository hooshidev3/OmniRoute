...
export function buildWorkerScript(kvNamespaceId: string, cloudSecret: string): string {
  return `// RouteChi Cloud Worker — auto-deployed
...
function buildModelsResponse(bundle) {
  ...
  for (const p of (bundle.providers || [])) {
    ...
    if (p.defaultModel && !models.has(p.defaultModel)) {
      models.set(p.defaultModel, { id: p.defaultModel, object: "model", created: now, owned_by: p.provider || "routechi" });
    }
  }
  if (bundle.modelAliases) {
    for (const alias of Object.keys(bundle.modelAliases)) {
      if (!models.has(alias)) models.set(alias, { id: alias, object: "model", created: now, owned_by: "routechi" });
    }
  }
  for (const c of (bundle.combos || [])) {
    if (c.name && !models.has(c.name)) models.set(c.name, { id: c.name, object: "model", created: now, owned_by: "routechi-combo" });
  }
...
export default {
  async fetch(request, env) {
    ...
    if (path === "/" || path === "") {
      return new Response(JSON.stringify({ service: "routechi-cloud", status: "ok", version: "1.0.0", timestamp: new Date().toISOString() }), { status: 200, headers: { "Content-Type": "application/json" } });
    }
