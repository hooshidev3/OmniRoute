 ...
 * ONLY from `npm info routechi version --json` (the `npm` CLI binary). When that binary
...
const NPM_REGISTRY_LATEST_URL = "https://registry.npmjs.org/routechi/latest";
...
const GITHUB_RELEASES_LATEST_URL =
  "https://api.github.com/repos/borhandarabi/routechi/releases/latest";
  ...
  try {
    const res = await fetchImpl(GITHUB_RELEASES_LATEST_URL, {
      ...
      headers: {
        ...
        "User-Agent": "routechi-version-check",
