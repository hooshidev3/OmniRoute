...
> **Agent-to-Agent Protocol v0.3** — Enables any AI agent to use RouteChi as an intelligent routing agent via JSON-RPC 2.0.

The A2A Server exposes RouteChi as a **first-class agent** that other agents can discover, delegate tasks to, and collaborate with using the [A2A Protocol](https://google.github.io/A2A/).
  ...
  "skills": [
    {
      "id": "smart-routing",
      "name": "Smart Routing",
      "description": "Routes prompts through RouteChi intelligent pipeline",
...
Routes prompts through RouteChi's intelligent pipeline with full observability.
...
Discovers RouteChi agent, sends a task, and processes the result.
"""
import requests
import json
...
```python
"""
LangChain integration — Use RouteChi A2A as a custom LLM.
"""
...
An orchestrator agent delegates code generation to RouteChi, then passes the output to a review agent.

```python
def coding_pipeline(task: str):
    # Step 1: Generate code via RouteChi A2A
    ...
    # Step 2: Review the code via RouteChi A2A (different model)
...
Multiple agents share quota through RouteChi, using the quota skill to coordinate.
