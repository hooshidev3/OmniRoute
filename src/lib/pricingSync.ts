...
// ─── Provider mapping: LiteLLM provider → RouteChi aliases ─────
...
/**
 * Transform LiteLLM raw data → RouteChi PricingByProvider format.
 *
 * Conversion: cost_per_token × 1_000_000 → $/1M tokens (RouteChi format).
 ...
 */
export function transformToRouteChi(raw: Record<string, LiteLLMModelInfo>): PricingByProvider {
  ...
  try {
    ...
    for (const source of validSources) {
      if (source === "litellm") {
        ...
        const transformed = transformToRouteChi(raw);
