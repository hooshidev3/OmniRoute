...
const INSTRUCTIONS = [
  "1. Install RouteChi's root certificate (Dashboard → AgentBridge → Cert)",
  ...
  "4. Open Kiro IDE — API calls will be automatically routed through RouteChi.",
  "5. Verify: check the Proxy Logs in RouteChi dashboard and look for provider=anthropic source=mitm",
];
...
export const KIRO_MITM_PROFILE: MitmTarget & {
  ...
  ...KIRO_TARGET,
  description:
    "Intercepts Kiro IDE requests to api.anthropic.com and routes them through RouteChi.",
  targetHost: HOSTS[0],
  targetPort: 443,
  localPort: 20130,
