...
const KILO_FREE_MODELS = [
  ...
  { id: "stepfun/step-3.7-flash:free", name: "StepFun Step 3.7 Flash (Free)" },
  {
    ...
    name: "NVIDIA Nemotron 3 Nano Omni 30B (Free, Reasoning)",
  },
  ...
  return (
    <div className="mt-6 rounded-lg border border-border p-4">
      ...
      <p className="mt-1 text-xs text-text-muted">
        Choose the model used when a request omits{" "}
        <code className="rounded bg-surface px-1 py-0.5 text-[11px]">model</code>. Default is{" "}
        <code className="rounded bg-surface px-1 py-0.5 text-[11px]">{DEFAULT_DEFAULT_MODEL}</code>{" "}
        (auto-routing).
      </p>
      ...
      <div className="mt-3 flex items-center gap-3">
        ...
        {saving && <span className="text-xs text-text-muted animate-pulse">Saving…</span>}
      </div>

      <p className="mt-3 text-[11px] text-text-muted">
        Tip: <code className="rounded bg-surface px-1 py-0.5">{DEFAULT_DEFAULT_MODEL}</code>{" "}
        auto-routes to the best available free backend. Reasoning models (
        <code className="rounded bg-surface px-1 py-0.5">nemotron</code>,{" "}
        <code className="rounded bg-surface px-1 py-0.5">deepseek-r1</code>) emit{" "}
        <code className="rounded bg-surface px-1 py-0.5">reasoning_content</code>.
      </p>
    </div>
  );
}
