...
export function SkillPreviewPane({
  ...
  const handleCopyRawUrl = useCallback(async () => {
    if (!skillId) return;
    const rawUrl = `https://raw.githubusercontent.com/diegosouzapw/RouteChi/refs/heads/main/skills/${skillId}/SKILL.md`;
    try {
  ...
  const githubUrl = skillId
    ? `https://github.com/diegosouzapw/RouteChi/blob/main/skills/${skillId}/SKILL.md`
    : null;
