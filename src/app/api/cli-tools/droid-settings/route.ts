...
// Check if settings has RouteChi customModels.
// Multi-model entries are stored as `custom:RouteChi-0`, `custom:RouteChi-1`, …
// (Ported from upstream PR decolua/9router#618.)
const hasRouteChiConfig = (settings: any) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(isRouteChiCustomModel);
};
...
// POST - Update RouteChi customModels (merge with existing settings)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Remove every existing RouteChi config (multi-model: index 0..N)
    settings.customModels = settings.customModels.filter((m) => !isRouteChiCustomModel(m));

    // Normalize baseUrl to ensure /v1 suffix
    const normalizedBaseUrl = baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1`;

    // Build and prepend RouteChi entries (one per requested model)
...
// DELETE - Remove RouteChi customModels only (keep other settings)
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove RouteChi customModels (every index, multi-model)
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter((m) => !isRouteChiCustomModel(m));
    ...
    return NextResponse.json({
      success: true,
      message: "RouteChi settings removed successfully",
    });
  } catch (error) {
